...
// Create a new order
orderRouter.post('/create', expressAsyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    for (const [productId, totalQuantity] of productQuantityMap.entries()) {
      ...
      if (!product) {
        ...
        if (product.stockAmount < totalQuantity) {
          res.status(400).send({ message: `Insufficient stock for product '${product.productName}'.` });
        }
    ...
    res.status(500).send({ message: 'An error occurred during order creation.' });
  }
}));
...
orderRouter.put(
  ...
  expressAsyncHandler(async (req: Request, res: Response) => {
    try {
      ...
      if (newOrderData.orderStatus === 'Cancelled' || newOrderData.orderStatus === 'Rejected by Seller') {
      ...
      const updatedOrder = await Order.findByIdAndUpdate(orderId, newOrderData, { new: true });
      ...
      res.status(500).send({
        message:
          "An error occurred when updating a specific order.",
      });
    }
  })
);
